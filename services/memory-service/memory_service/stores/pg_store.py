...
class Task(Base):
    ...
    input_data: Mapped[dict[str, Any] | None] = mapped_column("input", JSONB, default=dict)
    ...
    dependencies: Mapped[list[Any] | None] = mapped_column(JSONB, default=list)
    artifact_ids: Mapped[list[Any] | None] = mapped_column(JSONB, default=list)
    created_at: Mapped[str] = mapped_column(String, server_default=func.now())
...
class Artifact(Base):
    ...
    type: Mapped[str] = mapped_column(String(50), nullable=False)
    fmt: Mapped[str] = mapped_column("format", String(20), default="markdown")
